...
TEXTS: dict[str, dict[str, str]] = {
    ...
    "landing.subtitle": {
        "vi": "Rio nhận yêu cầu bảo trì qua chat, tự phân tích, lập kế hoạch, viết code trên nhánh dev và chỉ merge production khi <b>người duyệt</b> đồng ý.",
        "en": "Rio takes maintenance requests over chat, analyzes them, plans, writes code on a dev branch, and only merges to production when an <b>approver</b> agrees.",
        "ko": "Rio는 채팅으로 유지보수 요청을 받아 분석하고 계획을 세우며 dev 브랜치에 코드를 작성하고, <b>승인자</b>가 동의할 때만 프로덕션에 병합합니다."},
    ...
    "arch.sub": {
        "vi": "Rio đứng giữa cuộc trò chuyện của bạn và codebase: điều phối quy trình (FSM), để Claude Code suy nghĩ & viết code, nhưng <b>không bao giờ tự merge production</b> khi chưa có người duyệt.",
        "en": "Rio sits between your conversation and your codebase: it orchestrates the workflow (FSM), lets Claude Code think & write code, but <b>never merges to production on its own</b> without an approver.",
        "ko": "Rio는 대화와 코드베이스 사이에 위치합니다: 워크플로(FSM)를 조율하고 Claude Code가 사고하고 코드를 작성하게 하지만, 승인자 없이는 <b>절대 스스로 프로덕션에 병합하지 않습니다</b>."},
    ...
    "arch.node2.title": {"vi": "Rio Orchestrator", "en": "Rio Orchestrator", "ko": "Rio 오케스트레이터"},
    ...
    "mobile.feat1.desc": {"vi": "Báo lỗi bằng một tin nhắn; Rio lo phần còn lại và cập nhật tiến độ ngay trong thread.", "en": "Report a bug in one message; Rio handles the rest and posts progress right in the thread.", "ko": "메시지 하나로 버그를 신고하면 Rio가 나머지를 처리하고 스레드에 진행 상황을 게시합니다."},
    ...
    "mobile.step2.d": {"vi": "Rio đọc repo, tìm nguyên nhân gốc.", "en": "Rio reads the repo and finds the root cause.", "ko": "Rio가 저장소를 읽고 근본 원인을 찾습니다."},
    ...
    "iso.sub": {
        "vi": "Rio là nền tảng multi-tenant: code, bot và quyền của từng doanh nghiệp sống trong vùng cô lập của riêng họ — không bao giờ chạm vào nhau.",
        "en": "Rio is a multi-tenant platform: each company's code, bot and permissions live in their own isolated space — never touching each other.",
        "ko": "Rio는 멀티테넌트 플랫폼입니다: 각 회사의 코드, 봇, 권한이 자체 격리 공간에 존재하며 서로 닿지 않습니다."},
    ...
    "chan.sub": {
        "vi": "Không bắt cả team đổi thói quen. Rio nói chuyện qua kênh bạn đã dùng — hỗ trợ nhóm, thread và cộng tác nhiều người. Một thread = một yêu cầu bảo trì.",
        "en": "No need to change your team's habits. Rio talks over the channel you already use — with groups, threads and multi-person collaboration. One thread = one maintenance request.",
        "ko": "팀의 습관을 바꿀 필요가 없습니다. Rio는 이미 사용하는 채널에서 대화합니다 — 그룹, 스레드, 다중 사용자 협업을 지원합니다. 스레드 하나 = 유지보수 요청 하나."},
    "chan.tg.desc": {"vi": "Bot Rio chung không cần cài đặt, hoặc bot riêng mang tên & avatar thương hiệu của bạn.", "en": "The zero-setup shared Rio bot, or your own bot with your brand's name & avatar.", "ko": "설정이 필요 없는 공용 Rio 봇, 또는 브랜드 이름과 아바타가 있는 자체 봇."},
    ...
    "chan.fb.desc": {"vi": "Khách hàng & team nhắn Rio ngay trên Facebook Messenger của Trang doanh nghiệp.", "en": "Customers & team message Rio right on your business Page's Facebook Messenger.", "ko": "고객과 팀이 비즈니스 페이지의 Facebook Messenger에서 바로 Rio에게 메시지를 보냅니다."},
    ...
    "chan.sl.desc": {"vi": "Đội kỹ thuật nhắn Rio ngay trong Slack — nhận yêu cầu, duyệt kế hoạch, bấm nút phê duyệt không rời workspace.", "en": "Engineering teams message Rio right inside Slack — file requests, review plans, and approve with a click without leaving the workspace.", "ko": "엔지니어링 팀이 Slack에서 바로 Rio에게 메시지를 보냅니다 — 워크스페이스를 벗어나지 않고 요청·계획 검토·버튼 승인."},
    ...
    # ── Final CTA + footer ──
    "cta.title": {"vi": "Sẵn sàng để Rio lo phần bảo trì?", "en": "Ready to let Rio handle maintenance?", "ko": "Rio에게 유지보수를 맡길 준비가 되셨나요?"},
    ...
    "foot": {"vi": "🌙 Rio — Kỹ sư bảo trì AI · Bảo trì có kiểm soát, deploy có người duyệt.", "en": "🌙 Rio — AI Maintenance Engineer · Controlled maintenance, human-approved deploys.", "ko": "🌙 Rio — AI 유지보수 엔지니어 · 통제된 유지보수, 사람이 승인하는 배포."},
}
